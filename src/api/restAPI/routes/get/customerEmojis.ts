...
export const customerEmojis = async (
  req: Request,
  res: Response
): Promise<void> => {
  ...
  res.status(200).json(guild.emojis.cache.map((channel) => channel.toJSON()));
};

export const customerEmoji = async (
  req: Request,
  res: Response
): Promise<void> => {
  const client = req.app.get("client") as ExtendedClient;
  ...
  const emojiData = guild.emojis.cache.find((e) => e.name === emoji);
  ...
  res.status(200).json(emojiData.toJSON());
};
