...
export default {
    id: "permissions-commands-select",
        ...
        for (const commandName of interaction.values) {
            ...
            let data = await database.guildInteractionPermissions.findFirst({
                where: {
                    ...
                    Type: GuildPermissionType.COMMAND
                }
            })
                    ...
                    new ContainerBuilder()
                        .addTextDisplayComponents(
                            new TextDisplayBuilder()
                                .setContent(
                                    [`## ${await convertToEmojiToPng("terminal")} ${data.CommandName.split(".").join(" ")} (${data.UUID})`,
                                        ``,
                                        `**Interaction Name:** ${client.commands.get(data.CommandName)?.interactionName ?? "N/A"}`,
                                        `**Description:** ${client.commands.get(data.CommandName)?.interactionDescription ?? "N/A"}`,
