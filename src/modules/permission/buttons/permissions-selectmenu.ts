...
export default {
    ...
    async execute(interaction: ButtonInteraction, client: ExtendedClient) {
        ...
        const pageSize = 5;
        const data = client.selectmenus.map(value => value)

        if (data.length <= 0) return interaction.reply({
            content: "No selectmenus found.",
        ...
        const embedMessages = new TextDisplayBuilder()
            .setContent(
                (await Promise.all(list.map(async (l) => `**Selectmenu Name:** ${l.interactionName ?? "N/A"}${l.interactionDescription ? `\n**Description:** ${l.interactionDescription ?? "N/A"}` : ""}\n**CustomId:** ${l.customId ?? l.id}`))).join("\n\n")
            );

        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId("permissions-selectmenu-select")
            .setPlaceholder("Select a Option to manage")
            .addOptions(
                await Promise.all(list.map(async (l) => ({
                    label: `${l.interactionName ?? "N/A"} (${l.customId ?? l.id})`,
                    description: `CustomId: ${l.customId ?? l.id}`,
                    value: l.customId ?? l.id,
        ...
        const paginationData: PaginationData = {
            ...
            paginationData: data,
            buttonCustomId: "permissions-selectmenu",
