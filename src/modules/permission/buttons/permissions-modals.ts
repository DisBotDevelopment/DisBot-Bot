...
export default {
    ...
    async execute(interaction: ButtonInteraction, client: ExtendedClient) {
        ...
        const pageSize = 5;
        const data = client.modals.map(value => value)

        if (data.length <= 0) return interaction.reply({
            content: "No modals found.",
        ...
        const embedMessages = new TextDisplayBuilder()
            .setContent(
                (await Promise.all(list.map(async (l) => `**Modal Name:** ${l.interactionName ?? "N/A"}${l.interactionDescription ? `\n**Description:** ${l.interactionDescription ?? "N/A"}` : ""}\n**CustomId:** ${l.customId ?? l.id}`))).join("\n\n")
            );

        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId("permissions-modals-select")
        ...
        const paginationData: PaginationData = {
            ...
            paginationData: data,
            buttonCustomId: "permissions-modals",
