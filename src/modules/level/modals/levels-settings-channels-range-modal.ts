...
export default {
    id: "levels-settings-channels-range-modal",
    ...
    async execute(interaction: ModalSubmitInteraction, client: ExtendedClient) {
        try {
            ...
            await database.levelSettings.update({
                ...
                data: {
                    MessageXPRange: `${minInput}-${maxInput}`
                }
            })

            await sendDefaultMessage(`## ${await convertToEmojiToPng("check")} Updated the XP Range for Message XP.`, interaction, true, "reply")
