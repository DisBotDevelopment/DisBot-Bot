        ...
        try {
                ...
                return new EmbedBuilder()
                    ...
                    .setDescription(
                        [
                            ...
                            `**Handlers**: <@&${plainEmbed.Handlers}>`,
                            `**UUID**: \`\`\`${plainEmbed.CustomId}\`\`\``
                        ].join("\n")
                    );
            });

            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId("ticket-menu-manage-select")
                .setPlaceholder("Select a Option to manage")
                .addOptions(
                    embedsList.map((embed) => ({
                        label: embed.TicketChannelName || "Unknown Channel",
                        description: `UUID: ${embed.CustomId || "Unknown UUID"}`,
                        value: embed.CustomId || "unknown"
                    }))
                );
            ...
            const navigationRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
                new ButtonBuilder()
                    ...
                    .setCustomId(`ticket-menu-manage:${uuid}:${currentIndex - pageSize}`)
