...
export default {
    id: "button-ticket-close-archive-delete",
    ...
    async execute(interaction: ButtonInteraction, client: ExtendedClient) {
        ...
        const transcriptsChannel = guild?.channels.cache.get(transcripts as string);
        ...
        if (!transcriptsChannel) {
            interaction.reply({
                content: `## ${await convertToEmojiPng(
                    "info",
                    client.user.id
                )} Transscript channel is not set. Please contact the server owner to set it up.`,
        ...
        await (transcriptsChannel as TextChannel).send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setAuthor({
                        name: (member?.user.tag as string) ?? undefined,
