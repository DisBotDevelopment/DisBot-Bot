...
export default {
    ...
    async execute(interaction: ModalSubmitInteraction, client: ExtendedClient) {
        ...
        for (const ticketSetupDocument of fortickets) {
            ...
            if (!ticketSetupDocument)
                interaction.reply({
                    content: `## ${await convertToEmojiPng(
                        "check",
                        client.user.id
                    )} Please setup the ticket button first!`,
                });
            ...
            if (!blacklisted) throw new Error("Blacklisted member is not defined");
                            ...
                            .replace(
                                ...
                                (
                                    (await database.tickets.findMany({
                                        where: {
                                            SetupMessageId: interaction.message?.id as string,
                                        }
                                    })).length + 1
                                ).toString()
                            )}`,
