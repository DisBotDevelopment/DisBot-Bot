...
export default {
    id: "messages-message-components",
    ...
    async execute(interaction: ButtonInteraction, client: ExtendedClient) {
        ...
        let message;
        if (data.ComponentJSON != null) {
            const updatedComponents = await parseComponentData(data.ComponentJSON)
            message = await interaction.channel.send({
                flags: MessageFlags.IsComponentsV2,
                files: updatedComponents.files.length > 0 ? updatedComponents.files : [],
            ...
            message = await interaction.channel.send({
                ...
                components: [new ContainerBuilder()
                    .addTextDisplayComponents(
                        new TextDisplayBuilder().setContent("Edit this Component.")
                    )
                    .addTextDisplayComponents(
                        new TextDisplayBuilder().setContent("-# More Text in this.")
                    ),
                    new TextDisplayBuilder().setContent("-# More Text in this.")]
